...
1. Run `node scripts/audit-wiki-register.mjs` and review its non-blocking clustering warnings. Revise genuine rhetorical mannerism; do not mechanically eliminate every reported construction.
2. Run `node scripts/prepare-pages.mjs` and confirm it completes successfully.
3. Stage only the wiki files, linked top-level source files, and publishing infrastructure that belong to that change. Do not stage unrelated work.
4. Commit with a short message describing the completed wiki change.
5. Push the current branch to its configured upstream immediately. For normal wiki work this is `main` to `origin`.
6. If the push is rejected because the remote advanced, fetch and integrate the remote changes without discarding local work, rerun the register audit and Pages preparation check, and push again.
